...
class Pipe:
    ...
    def get_regular_reporting(self, base_date, target_date=None):
        if target_date is None:
            db.ResultListDataStore(self.get_list(bgn_de=base_date, pblntf_ty=self.pblntf_ty_codes["정기공시"],
                                                 pblntf_detail_ty=self.pblntf_detail_ty_codes["사업보고서"]))
            # 결과는 이상하게 동일하기 때문에 아래는 주석처리
            # db.ResultListDataStore(self.get_list(bgn_de=base_date, pblntf_ty=self.pblntf_ty_codes["정기공시"],
            #                                      pblntf_detail_ty=self.pblntf_detail_ty_codes["반기보고서"]))
            # db.ResultListDataStore(self.get_list(bgn_de=base_date, pblntf_ty=self.pblntf_ty_codes["정기공시"],
            #                                      pblntf_detail_ty=self.pblntf_detail_ty_codes["분기보고서"]))
        else:
            db.ResultListDataStore(
                self.get_list(bgn_de=base_date, end_de=target_date, pblntf_ty=self.pblntf_ty_codes["정기공시"],
                              pblntf_detail_ty=self.pblntf_detail_ty_codes["사업보고서"]))
            # 결과는 이상하게 동일하기 때문에 아래는 주석처리
            # db.ResultListDataStore(
            #     self.get_list(bgn_de=base_date, end_de=target_date, pblntf_ty=self.pblntf_ty_codes["정기공시"],
    ...
    def get_regular_reporting_corp_info(self, rcept_no, base_date, target_date=None):
        corp_code_list = None
        if target_date is None:
            # self.get_regular_reporting(base_date)
            # if rcept_no:
            #     corp_code_list = db.getRegularReportingInfo(rcept_no, base_date)
            # else:
            corp_code_list = db.getRegularReportingInfo(rcept_no, base_date)
        else:
            # self.get_regular_reporting(base_date, target_date)
            # if rcept_no:
            #     corp_code_list = db.getRegularReportingInfo(rcept_no, base_date, target_date)
            # else:
            corp_code_list = db.getRegularReportingInfo(rcept_no, base_date, target_date)
        print(corp_code_list)
                ...
                if "list" in ret.keys():
                    for l in ret["list"]:
                        if l["account_id"] in ["ifrs-full_CashFlowsFromUsedInOperatingActivities",
                                               ...
                                               "ifrs-full_Revenue", "ifrs-full_Assets", "ifrs-full_Liabilities",
                                               "dart_OperatingIncomeLoss", "ifrs-full_RetainedEarnings"
                            ...
                            if l["account_id"] == "ifrs-full_Assets": account_name = "자산총계"
                            if l["account_id"] == "ifrs-full_Liabilities": account_name = "부채총계"
                            if l["account_id"] == "ifrs-full_RetainedEarnings": account_name = "이익잉여금"
                        else:
...
if __name__ == "__main__":
    ...
    dart.get_regular_reporting(date)
    ...
    # ret, code = dart.get_corp_code('306620')
